...
2.  **Создаём новую крепость (репозиторий):**
    ...
    * **![Описание](../images/890dfg89udfg9jip.png)**
...
2.  **Вставляем содержимое скрипта:**
    ...
    # Устанавливаем новые зависимости (если requirements.txt изменился) 📦
    ...
    * **ОЧЕНЬ ВАЖНО:**
        ...
        * Замени `<ИМЯ_ПАПКИ_ПРОЕКТА>` в строке `PROJECT_DIR="/root/<ИМЯ_ПАПКИ_ПРОЕКТА>"` на **реальное имя твоей папки проекта** (например, `my_bot_project`).
...
2.  **Добавляем строку для расписания (говорим будильнику, когда звонить):**
    ...
    * **ОБЯЗАТЕЛЬНО замени `<ИМЯ_ПАПКИ_ПРОЕКТА>` на реальное имя твоей папки проекта!**
...
| Команда | Описание | Когда это нужно? |
| :---------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------- |
| `sudo systemctl status <ИМЯ_ТВОЕГО_СЕРВИСА>.service` | Покажет текущий статус бота: работает ли он (🟢 `active (running)`) или нет. Подскажет, почему он вдруг решил поспать. 😴💤 | Когда бот молчит, как партизан, и ты не понимаешь, что с ним. 🤫 |
| `sudo systemctl stop <ИМЯ_ТВОЕГО_СЕРВИСА>.service` | Остановит бота. Это как нажать на кнопку "пауза" ⏸️, чтобы дать ему передохнуть. | Если тебе нужно вручную что-то быстро изменить или поправить, и ты не хочешь, чтобы бот мешался. 🛠️ |
| `sudo systemctl start <ИМЯ_ТВОЕГО_СЕРВИСА>.service` | Запустит бота. Это как сказать "Поехали!" 🚀 — и твой бот снова в строю! | После того, как ты его остановил, или если он почему-то сам не запустился. 🚦 |
| `sudo systemctl restart <ИМЯ_ТВОЕГО_СЕРВИСА>.service` | Перезапустит бота. Это как вежливое "Пожалуйста, обновись и начни заново" 👋. Чистый лист для свежих идей! | Самая частая команда! Пригодится, когда ты вручную обновил код или конфиг, и хочешь, чтобы бот подхватил изменения. ✨ |
| `sudo systemctl enable <ИМЯ_TВОЕГО_СЕРВИСА>.service` | Включит бота в автозагрузку. Он будет стартовать вместе с сервером. 🏎️ Без напоминаний! | Если ты только что настроил сервис или хочешь убедиться, что он всегда будет запускаться, даже после перезагрузки сервера. ✅ |
| `sudo systemctl disable <ИМЯ_ТВОЕГО_СЕРВИСА>.service` | Отключит бота из автозагрузки. Он не будет стартовать вместе с сервером. 🚶‍♂️ Пусть отдохнёт! | Если ты временно не хочешь, чтобы бот запускался сам, или если ты его переносишь на новое место. 🏠 |

#### 📝 На сервере: Просмотр логов (журнал приключений бота 📖)

| Команда | Описание | Когда это нужно? |
| :------------------------------------------------ | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------- |
| `sudo journalctl -u <ИМЯ_ТВОЕГО_СЕРВИСА>.service -f` | Покажет последние логи бота в реальном времени. Это как подглядывать в его дневник! 🤫 Тут вся его жизнь, включая ошибки. 🚨 | Когда бот что-то делает не так, и тебе нужно понять, в чем проблема. `-f` означает "следить" за новыми записями, как за новостной лентой. 📰 |
| `cat /root/<ИМЯ_ПАПКИ_ПРОЕКТА>/update_log.log` | Просмотр лога автообновлений. Здесь наш внутренний автомат 🤖 записывает, как он старается обновлять код. Всё по пунктам! 📋 | Когда кажется, что автообновление не работает, или чтобы убедиться, что всё прошло гладко, как по маслу. 🧈 |
| `tail -f /root/<ИМЯ_ПАПКИ_ПРОЕКТА>/update_log.log` | То же самое, что и `cat`, но показывает только конец файла и следит за обновлениями. Удобно, когда хочешь видеть новые записи без лишнего шума. 🔇 | Актуально, когда ты ждешь, пока `cron` запустит скрипт, и хочешь увидеть результат в прямом эфире. 📺 |

#### ⚙️ На сервере: Ручное обновление проекта (Дёргаем за нужные ниточки! 🧵)

| Команда | Описание | Когда это нужно? |
| :------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cd /root/<ИМЯ_ПАПКИ_ПРОЕКТА>` | Переход в папку проекта. 🚶‍♂️ Твоя рабочая база! | Перед выполнением любых действий с файлами проекта, чтобы не заблудиться. 🗺️ |
| `bash update_project.sh` | Запустит скрипт обновления вручную. Это как сказать автомату: "Давай, работай прямо сейчас!" ⚙️ Вне очереди! | Если тебе нужно срочно обновить код, не дожидаясь `cron` (который обычно запускается раз в 5 минут). ⏱️ |

#### 📁 На сервере: Работа с файлами (Твоя цифровая кладовка 🗄️)

| Команда | Описание | Когда это нужно? |
| :-------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ls -la` | Покажет все файлы и папки в текущей директории, включая скрытые (`.`). 🕵️‍♀️ Ничего не скроешь! | Чтобы убедиться, что нужные файлы (например, `.gitignore`, `config.py`) на месте и имеют правильные права. ✅ |
| `nano <ИМЯ_ФАЙЛА>` | Откроет файл в текстовом редакторе `nano`. ✍️ Простой и удобный, как карандаш. ✏️ | Для быстрого редактирования `config.py`, `requirements.txt` или других файлов прямо на сервере. |

#### ➕ На сервере: Дополнительно (Аптечка для сервера 💊)

| Команда | Описание | Когда это нужно? |
| :---------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sudo apt update && sudo apt upgrade -y` | Обновит пакеты системы. Всегда полезно держать сервер в тонусе, как себя после утренней зарядки. 💪☀️ | Раз в несколько недель/месяцев для поддержания актуальности системы и её безопасности. 🛡️ |
| `sudo reboot` | Перезагрузит весь сервер. Это как перезагрузить компьютер, когда он завис и отказывается работать. 🔄 Последний аргумент! | Только если ничего другое не помогает или после глобальных обновлений ядра системы. 🧠 |

#### 💻 На локальном компьютере (для GitHub): Синхронизация и команды Git (Твой личный Git-командир 💂‍♂️)

| Команда | Описание | Когда это нужно? |
| :------------------------------------------------ | :--------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------- |
| `git add .` | Добавляет все изменения в "список на отправку". 📝 Это как собрать вещи в рюкзак перед походом. 🎒 | Перед каждым коммитом, чтобы включить все измененные файлы в новую версию. |
| `git commit -m "Твое крутое сообщение об изменениях"` | Создаёт "снимок" твоего проекта с комментарием. 📸 Это как подписать фотографию, чтобы потом вспомнить, что на ней. 🖼️ | После того, как ты сделал изменения и хочешь их сохранить в истории версий. |
| `git push origin main` | Отправляет твои "снимки" в крепость на GitHub. 🚀 Это как запустить ракету в космос! 🌌 | После коммита, чтобы твои изменения стали доступны для сервера (и твоих коллег, если они есть). |
| `git pull origin main` | Забирает самые свежие изменения из GitHub на твой локальный компьютер. 📥 Это как получить посылку с подарками! 🎁 | Если ты работаешь в команде и хочешь получить код других разработчиков, или если ты что-то поменял прямо на GitHub. |
| `git status` | Покажет текущее состояние твоего репозитория: что изменено, что не добавлено, что готово к коммиту. 🚦 Твой личный навигатор по коду. | Чтобы понять, что происходит в твоём проекте перед `git add` или `git commit`. |
...
4.  **Ждёшь, пока сработает `cron`:** Твой `cron` настроен на запуск `update_project.sh` каждые 5 минут. Так что просто налей себе чаю ☕, возьми печеньку 🍪 и подожди. Магия произойдёт сама! ✨
...
3.  **Расшифруй ошибку:**
    ...
    * **"Permission denied"**: У бота нет прав что-то сделать. 🔒 Проверь, что скрипт `update_project.sh` исполняемый (`chmod +x`), и что пользователь, от которого запускается сервис (обычно `root` или твой пользователь), имеет права на чтение/запись в папке проекта. 🔑
