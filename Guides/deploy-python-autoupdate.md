...
1.  [Твоя тайная крепость на GitHub и волшебный ключ для автообновлений](#1-твоя-тайная-крепость-на-github-и-волшебный-ключ-для-автообновлений)
...
2.  [Заселяем бота на сервер: Большая уборка и первый запуск](#2-заселяем-бота-на-сервер-большая-уборка-и-первый-запуск)
    * [2.1. Обновление и нужные инструменты: Готовимся к встрече 🛠️](#21-обновление-и-нужные-инструменты-готовимся-к-встрече-️)
...
3.  [Бот-автомат: Пусть обновляется сам!](#3-бот-автомат-пусть-обновляется-сам)
...
4.  [Когда что-то пошло не так: Шерлок Холмс в мире кода](#4-когда-что-то-пошло-не-так-шерлок-холмс-в-мире-кода)
...
5.  [Заключение: Твой бот — теперь супергерой! 🦸‍♂️](#5-заключение-твой-бот-теперь-супергерой)
...
* **`sudo systemctl stop <ИМЯ_ТВОЕГО_СЕРВИСА>.service`**
  ...
  * **Описание:** Остановит бота. Это как нажать на кнопку "пауза" ⏸️, чтобы дать ему передохнуть.
...
* **`git add .`**
  ...
  * **Когда это нужно?** Перед каждым коммитом, чтобы включить все измененные файлы в новую версию.
