...
5.  **Пишем "план строительства" (`docker-compose.yml`) 📝**
    ...
    **Важно:** Для каждого тестового сервера тебе понадобится свой уникальный SOCKS5-прокси, а также правильные логин и пароль.
    ...
    services:
      ...
      s1: # Пример имени сервиса, можете изменить на свое
        build: .
        # Docker Compose: "Бери Dockerfile отсюда и собирай образ!" 🏭
        container_name: ubuntu_s1
        ...
        environment:
          ...
          HTTP_PROXY: "socks5://<ЛОГИН_ПРОКСИ_1>:<ПАРОЛЬ_ПРОКСИ_1>@<IP_АДРЕС_ПРОКСИ_1>:1080" # ЗАМЕНИ
          # HTTPS-звонки через прокси. Обычно то же самое.
          HTTPS_PROXY: "socks5://<ЛОГИН_ПРОКСИ_1>:<ПАРОЛЬ_ПРОКСИ_1>@<IP_АДРЕС_ПРОКСИ_1>:1080" # ЗАМЕНИ
      ...
      # Он будет использовать СВОЙ УНИКАЛЬНЫЙ SOCKS5-прокси
      s2: # Пример имени сервиса, можете изменить на свое
        build: .
        # Тот же Dockerfile, чтобы они были одинаковыми.
        container_name: ubuntu_s2
        # Имя контейнера, чтобы легко его находить
        ports:
          - "2222:22"
        ...
        environment:
          ...
          HTTP_PROXY: "socks5://<ЛОГИН_ПРОКСИ_2>:<ПАРОЛЬ_ПРОКСИ_2>@<IP_АДРЕС_ПРОКСИ_2>:1080" # ЗАМЕНИ
          HTTPS_PROXY: "socks5://<ЛОГИН_ПРОКСИ_2>:<ПАРОЛЬ_ПРОКСИ_2>@<IP_АДРЕС_ПРОКСИ_2>:1080" # ЗАМЕНИ
      ...
      # Он будет использовать СВОЙ УНИКАЛЬНЫЙ SOCKS5-прокси
      s3: # Пример имени сервиса, можете изменить на свое
        build: .
        container_name: ubuntu_s3
        ports:
          - "2223:22"
        networks:
          my_test_network:
        ...
        environment:
          ...
          HTTP_PROXY: "socks5://<ЛОГИН_ПРОКСИ_3>:<ПАРОЛЬ_ПРОКСИ_3>@<IP_АДРЕС_ПРОКСИ_3>:1080" # ЗАМЕНИ
          HTTPS_PROXY: "socks5://<ЛОГИН_ПРОКСИ_3>:<ПАРОЛЬ_ПРОКСИ_3>@<IP_АДРЕС_ПРОКСИ_3>:1080" # ЗАМЕНИ
...
4.  **Проверяем IP внутри контейнера (самый важный тест!):**
    ...
    curl ifconfig.me
    ```
    * **Что ты увидишь**: Должен отобразиться **IP-адрес твоего SOCKS5-прокси** (`<IP_АДРЕС_ПРОКСИ_1>`, `<IP_АДРЕС_ПРОКСИ_2>` или `<IP_АДРЕС_ПРОКСИ_3>`), а не твой домашний IP. Также ты увидишь отладочные сообщения `|S-chain|`, подтверждающие работу `proxychains`.
...
2.  **Зайти внутрь контейнера для ручной проверки (отладочный режим!):**
    ...
    * **Внутри контейнера** ты можешь выполнять команды Linux, например:
        ...
        * `find / -name "libproxychains.so*"`: Найти библиотеку `libproxychains.2.0.0` (например).
...
1.  **Заходим в контейнер для настройки:**
    Сначала вам нужно попасть в командную строку нужного контейнера. Вы можете сделать это двумя способами:
    * **Через SSH:**
        ```bash
        ssh root@localhost -p 2221 # Для m1, используйте 2222 для m2, 2223 для m3
        ```
        Введите ваш **СЕКРЕТНЫЙ_SSH_ПАРОЛЬ**.
    * **Через `docker exec` (если SSH не работает или для быстрой отладки):**
        ```bash
        docker exec -it ubuntu_m1 bash # Для ubuntu_m1, используйте ubuntu_m2, ubuntu_m3
        ```

2.  **Обновляем систему (это важно!):**
...
3.  **Устанавливаем полезные утилиты:**
    Эти программы сделают работу в консоли намного комфортнее:
    * `htop`: Красивый "диспетчер задач" для Linux.
    * `mc` (Midnight Commander): Файловый менеджер в консоли, очень удобный для навигации.
...
5.  **Настраиваем брандмауэр UFW (основа безопасности):**
    ...
    * Разрешаем входящие SSH-подключения (порт 22):
        ...
        ufw allow ssh
        ```
    * Разрешаем входящие SOCKS5-подключения (порт 1080):
        ```bash
        ufw allow 1080/tcp
        ```
...
6.  **Создаем нового пользователя (это безопасно и удобно):**
    ...
    Теперь дайте новому пользователю права `sudo`:
    ```bash
    usermod -aG sudo <НОВОЕ_ИМЯ_ПОЛЬЗОВАТЕЛЯ>
    ```
    После этого вы можете выйти из сессии `root` (`exit`) и подключиться к контейнеру уже под новым пользователем:
    ```bash
...
7.  **Настраиваем приветствие (Message of the Day - MOTD):**
    ...
    echo "" >> /etc/motd # Пустая строка для разделения
    echo "Ваш внешний IP через прокси: $(curl -s ifconfig.me)" >> /etc/motd
    echo "Для использования прокси: socks5://<ЛОГИН_ПРОКСИ>:<ПАРОЛЬ_ПРОКСИ>@<IP_АДРЕС_ПРОКСИ>:1080" >> /etc/motd
    ```
    * **ВНИМАНИЕ:** Замените `<ЛОГИН_ПРОКСИ>`, `<ПАРОЛЬ_ПРОКСИ>` и `<IP_АДРЕС_ПРОКСИ>` на реальные данные для этого конкретного контейнера! Эти данные зависят от того, к какому контейнеру вы подключились (`m1`, `m2` или `m3`)!
    Чтобы проверить, как выглядит MOTD:
    ```bash
    cat /etc/motd
    ```

8.  **Перезапускаем SSH-сервер (для применения изменений):**
