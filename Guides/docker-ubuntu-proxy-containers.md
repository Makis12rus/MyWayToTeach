...
7.  **Настраиваем приветствие (Message of the Day - MOTD):**
    ...
    echo "Для использования прокси: socks5://<ЛОГИН>:<ПАРОЛЬ>@<IP_АДРЕС_ПРОКСИ>:1080" >> /etc/motd
    ```
    * **ВНИМАНИЕ:** Замените `<ЛОГИН>`, `<ПАРОЛЬ>` и `<IP_АДРЕС_ПРОКСИ>` на реальные данные для этого конкретного контейнера!
